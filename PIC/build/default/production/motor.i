...
# 7 "motor.c" 2
# 24 "motor.c"
static volatile _Bool g_motor_running = 0;
static volatile _Bool g_step_pin_state = 0;
static volatile uint16_t g_tmr1_reload = 0U;

static uint16_t motor_timer1_reload_from_interrupt_hz(float interrupt_hz)
{
    float tick_hz = ((float)(32000000UL) / 4.0f) / 8.0f;
...
void motor_isr(void)
{
    if ((PIE1bits.TMR1IE != 0U) && (PIR1bits.TMR1IF != 0U))
    {
        ...
        if (g_motor_running)
        {
            ...
            LATCbits.LATC1 = (g_step_pin_state ? 1U : 0U);
        }
        else
        {
            g_step_pin_state = 0;
...
void motor_start_for_flow_ul_min(float flow_ul_min)
{
    float motor_steps_per_rev_effective = (200.0f) * (64.0f);
    ...
    float rev_per_min = flow_ul_min / (52.5f);
