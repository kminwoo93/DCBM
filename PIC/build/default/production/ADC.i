...
float ADC_Get_LMP_RTIA_Ohms(void)
{
    uint8_t g = (uint8_t)(((0x18U) | (0x03U)) & (0x1Cu));
    switch (g) {
        case (0x00U): return 0.0f;
        case (0x04U): return 2750.0f;
        case (0x08U): return 3500.0f;
        case (0x0CU): return 7000.0f;
        case (0x10U): return 14000.0f;
        case (0x14U): return 35000.0f;
        case (0x18U): return 120000.0f;
        case (0x1CU): return 350000.0f;
...
uint16_t ADC_Get_LMP_Vref_mV(void)
{
    ...
    _Bool ext = ((((0x00U) | (0x40U) | (0x00U) | (0x02U)) & (0x80u)) != 0);
    if (ext) {

        return 2500;
    } else {

        return (uint16_t)3300;
    }
}

static float LMP_GetIZ_Fraction(void)
{
    uint8_t iz = (uint8_t)(((0x00U) | (0x40U) | (0x00U) | (0x02U)) & (0x60u));
    switch (iz) {
        case (0x00U): return 0.20f;
        case (0x20U): return 0.50f;
        case (0x40U): return 0.67f;
...
static __attribute__((inline)) void adc_acquire_delay_us(uint16_t us)
{
    while (us--) { _delay((unsigned long)((1)*((32000000UL)/4000000.0))); }
}
...
uint16_t ADC_CodeToMilliVolts(uint16_t code)
{
    ...
    const uint32_t vref_mV = (uint32_t)2048;
...
float ADC_CodeToCurrent_uA(uint16_t code)
{
    const float vref_mV = 2048.0f;
    const float vzero_mV = 1650.0f;
    const float rtia_ohms = 350000.0f;
