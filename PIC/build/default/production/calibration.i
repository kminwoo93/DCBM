...
# 5 "D:\\XC8\\pic\\include\\c99/stdint.h" 2 3
...
{
    float known_conc_mM;
    cal_measurement_t replicate_a;
    cal_measurement_t replicate_b;
    float avg_current_nA;
    _Bool valid;
} cal_point_t;
# 75 "./calibration.h"
typedef struct
...
typedef struct
{
    cal_point_t points[3U];
    cal_regression_t regression;
    _Bool calibrated;
} cal_analyte_t;
...
typedef struct
{
    ...
    cal_analyte_t analyte[2U];
    cal_blood_sample_t blood[2U];
} cal_state_t;



void cal_init(cal_state_t *cal);
_Bool cal_measure_pbs_baseline(cal_state_t *cal);
_Bool cal_measure_cal_point(cal_state_t *cal, uint8_t point_index, float glucose_conc_mM, float lactate_conc_mM);
_Bool cal_compute_regression(cal_state_t *cal);
_Bool cal_measure_blood_sample(cal_state_t *cal, uint8_t analyte_index, float known_conc_mM);
...
# 10 "D:\\XC8\\pic\\include\\c99/stdlib.h" 3
...
void run_measurement_loop(uint16_t duration_seconds, uint16_t delay_ms);
# 7 "calibration.c" 2
# 1 "./uart.h" 1
# 19 "./uart.h"
void uart_init(void);
...
{
    motor_start_for_flow_ul_min(100.0f);
    ...
    _delay((unsigned long)((2000UL)*(_XTAL_FREQ/4000.0)));
}


static void measure_channel(uint8_t ch_num, _Bool is_ocp, cal_measurement_t *out)
{
    char buf[32];
    ...
    for (uint8_t i = 0U; i < 10U; i++)
    {
        ...
        _delay((unsigned long)((100U)*(_XTAL_FREQ/4000.0)));
    }

    out->adc_avg = (uint16_t)(adc_sum / 10U);
...
_Bool cal_measure_pbs_baseline(cal_state_t *cal)
{
    ...
    run_motor_dispense();
# 147 "calibration.c"
    measure_channel(1U, 0, &cal->baseline.ch[0]);
    measure_channel(2U, 0, &cal->baseline.ch[1]);
    measure_channel(3U, 1, &cal->baseline.ch[2]);
    measure_channel(4U, 0, &cal->baseline.ch[3]);
    measure_channel(5U, 0, &cal->baseline.ch[4]);
    measure_channel(6U, 0, &cal->baseline.ch[5]);

    cal->baseline.complete = 1;
    return 1;
}
...
_Bool cal_measure_cal_point(cal_state_t *cal, uint8_t point_index, float glucose_conc_mM, float lactate_conc_mM)
{
    ...
    cal_point_t *glu = &cal->analyte[0U].points[point_index];
    glu->known_conc_mM = glucose_conc_mM;

    measure_channel(2U, 0, &glu->replicate_a);
    measure_channel(5U, 0, &glu->replicate_b);
    ...
    cal_point_t *lac = &cal->analyte[1U].points[point_index];
    lac->known_conc_mM = lactate_conc_mM;

    measure_channel(3U, 1, &lac->replicate_a);
    measure_channel(6U, 0, &lac->replicate_b);

    int32_t lac_a_net = lac->replicate_a.current_nA
                        - cal->baseline.ch[2].current_nA;
    int32_t lac_b_net = lac->replicate_b.current_nA
                        - cal->baseline.ch[5].current_nA;
...
_Bool cal_compute_regression(cal_state_t *cal)
{
    ...
    for (uint8_t a = 0U; a < 2U; a++)
    {
        ...
        linear_regression(x, y, 3U, &slope, &intercept, &r2);
        ...
        cal->analyte[a].regression.valid = 1;
        cal->analyte[a].calibrated = 1;

        const char *name = (a == 0U) ? "Glucose" : "Lactate";
    }

    return 1;
}

_Bool cal_measure_blood_sample(cal_state_t *cal,
                              ...
                              float known_conc_mM)
{
    ...
    const char *name = (analyte_index == 0U) ? "Glucose" : "Lactate";
    ...
    if (analyte_index == 0U)
    {
        measure_channel(2U, 0, &blood->replicate_a);
        measure_channel(5U, 0, &blood->replicate_b);
    }
    else
    {
        measure_channel(3U, 1, &blood->replicate_a);
        measure_channel(6U, 0, &blood->replicate_b);
    }
...
float cal_calculate_concentration(const cal_state_t *cal,
                                  ...
                                  float current_nA)
{
    if (analyte_index >= 2U) return -1.0f;
...
void cal_print_summary(const cal_state_t *cal)
{
    ...
    const char *names[2U] = {"Glucose", "Lactate"};
    for (uint8_t a = 0U; a < 2U; a++)
    {
        for (uint8_t p = 0U; p < 3U; p++)
        {
