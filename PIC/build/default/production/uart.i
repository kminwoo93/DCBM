...
volatile _Bool g_uart_rx_ready = 0;
# 43 "uart.c"
static uint8_t receive_buffer[(32U)];
static uint8_t receive_index = 0U;


static uint16_t uart_calculate_brg_value(void)
{
    const uint32_t divisor = 4UL * (115200UL);
    uint32_t value = ((32000000UL) + (divisor / 2UL)) / divisor;
    if (value > 0U)
    {
        value -= 1U;
    }
...
void uart_init(void)
{
    ...
    SP1BRGH = (uint8_t)(brg_value >> 8);
    SP1BRGL = (uint8_t)(brg_value & 0xFFU);
...
_Bool uart_try_read_line(char *buffer, uint8_t buffer_len)
{
    if ((buffer == ((void*)0)) || (buffer_len == 0U))
    {
        return 0;
    }
...
_Bool uart_try_read_byte(uint8_t *b)
{
